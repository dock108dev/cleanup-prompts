...
Retain the existing visual direction and components where they work. Read the shared `ui-templates` requirements if available, or the repository's local copy. When available, start from the shared Starter 02 working examples and matched comparison; the original Desktop gallery is currently unavailable, so use the target repository’s portable requirements and existing components. Check the app's adoption version: a Starter 01 local copy does not already include these improvements. Adapt the layout to the task; a decorative template must not dictate the amount of chrome.
